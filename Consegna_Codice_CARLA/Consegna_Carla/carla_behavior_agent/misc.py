...
def is_within_distance(target_transform, reference_transform, max_distance, angle_interval=None):
    ...
    # print("la targer transform è:", target_transform.location)
    # print("la reference transform è:", reference_transform.location)
    # print("la norma tra i due punti è:", norm_target)
...
def compute_length(actor):
    """
    Compute the length of an actor

        :param actor: the actor to compute the length of
    """
    return max(actor.bounding_box.extent.y, actor.bounding_box.extent.x) * 2
    #return np.linalg.norm(actor.get_location().location - actor.get_transform().location.location)

def is_bike(actor):
    if not actor.type_id.startswith('vehicle.bh') and not actor.type_id.startswith('vehicle.diamondback') and not actor.type_id.startswith('vehicle.gazelle'):
        return False
    return True



'''bad_waypoint= [15738739296028174304,
                 1410879750131066464,
                 5580584497298346465,
                 3838883643220318192,
                 17106385606845221635,
                 10520321003431969191,
                 469566937191134233,
                 7109300839316733915,
                 14197533696397744381,
                 15287740456859642588,
                 14366161302312436734,
                 3235858135203318126,
                 16834850132066987602,
                 13216897762278854404]

def fix_waypoint(wp, bad_waypoints):
    if wp.id in bad_waypoints:
        wp.is_junction = False
        print("Done Fixing Waypoint: ", wp.id)
        '''
